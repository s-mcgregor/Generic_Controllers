...
disp('Homework 4 Hinf SF')
 ...
 HINF.y      = y;
 HINF.t      = t;
 HINF.az     = y(:,1); %  acceleration (fps2)
 HINF.taur   = taur;
 HINF.taus   = taus;
 HINF.Lu     = Lu;
 HINF.rd     = rd;
 HINF.rdu_min   = rdu_min;
 HINF.sr     = sr;
 HINF.sru_min   = sru_min;
 HINF.S      = S;
 HINF.T      = T;
...
plot(t,az,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,1),'k','LineWidth',2);grid
legend(['Hinf 63% Tr = ' num2str(taur) ' 95% Ts = ' num2str(taus)],...
       ['Hmk3 63% Tr = ' num2str(Hmk3_RSLQR.metric(4)) ' 95% Ts = ' num2str(Hmk3_RSLQR.metric(5))],...
       'Location','Best');
xlabel('Time (sec)');
ylabel('Az (fps2)');
if(save_plots == 1) saveppt2(plot_file_name); end
...
plot(t,y(:,3)*rtd,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,3)*rtd,'k','LineWidth',2);grid
legend('Hinf SF ', ...
       'Hmk3 ',...
       'Location','Best');
xlabel('Time (sec)');
...
plot(t,y(:,4)*rtd,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,4)*rtd,'k','LineWidth',2);grid
legend('Hinf SF ', ...
       'Hmk3 ',...
       'Location','Best');
xlabel('Time (sec)');
...
plot(t,y(:,5)*rtd,'b',Hmk3_RSLQR.t, Hmk3_RSLQR.y(:,5)*rtd,'k','LineWidth',2);grid
legend('Hinf SF ', ...
       'Hmk3 ',...
       'Location','Best');
xlabel('Time (sec)');
...
plot(xx1,yy1,'r',real(squeeze(Lu)),imag(squeeze(Lu)),'b',...
real(Hmk3_RSLQR.Lu),imag(Hmk3_RSLQR.Lu),'k','LineWidth',2);grid
axis([-2 2 -2 2]);
...
semilogx(w,20*log10(abs(squeeze(Lu))),'b',...
    w,20*log10(abs(Hmk3_RSLQR.Lu)),'k','LineWidth',2);grid
legend([ 'Hinf-SF LGCF = '     num2str(HINF.metric(4)) ],...
       [ 'Hmk3 RSLQR LGCF = '  num2str(Hmk3_RSLQR.metric(3)) ],...
       'Location','Best');
...
semilogx(w,20*log10(abs(rd)),'b',w,20*log10(abs(Hmk3_RSLQR.rd)),'k','LineWidth',2);grid
legend([' Hinf min(I+Lu) = ' num2str(rdu_min)],...
       ['Hmk3 RSLQR I+Lu min = ' num2str(Hmk3_RSLQR.metric(1)) ],...
       'Location','Best');
xlabel('Frequency (rps)')
ylabel('Mag dB')
...
semilogx(w,20*log10(abs(sr)),'b',w,20*log10(abs(Hmk3_RSLQR.sr)),'k', 'LineWidth',2);grid
legend([' Hinf min(I+Lu) = ' num2str(sru_min)],...
...
semilogx(w,HINF.T,'b',w ,Hmk3_RSLQR.T,'k');grid
legend(['||T||inf = ' num2str(Tmax) ' (dB)'],...
        'invWt',...
...
tex2 = ['T-Az norm RSLQR= ' num2str(T_az_max_RSLQR)];
...
%{
figure('Name','Sens S');
semilogx(w,S_Az,'b',w,Ws_magdb,'g',w,Hmk3_RSLQR.S,'k');grid
legend(['||S||inf = ' num2str(Smax) ' (dB)'],...
        'invWs',...
...
tex2 = ['S-Az norm RSLQR= ' num2str(S_az_max_RSLQR)];
legend(tex1, tex2, 'Location','Best');
xlabel('Frequency (rps)')
ylabel('Co-Sens Mag (dB)')
hold off
